    ...
    async getPrediction() {
      ...
      try {
        const responses = await Promise.all([
          fetch(` http://192.168.50.15:5000/current-price?ticker=${ticker}`),
          fetch(` http://192.168.50.15:5000/predict?ticker=${ticker}&days=90`),
          fetch(` http://192.168.50.15:5000/predict-image?ticker=${ticker}&days=90`)
        ]);
