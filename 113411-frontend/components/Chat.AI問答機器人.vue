...
export default {
  ...
  data() {
    return {
      userInput: '',
      messages: [], // 初始設為空，稍後在 mounted 中加載
      isChatOpen: false, // 初始設為 false，稍後在 mounted 中加載
    };
  },
  mounted() {
    // 在 mounted 鉤子中加載本地儲存的聊天視窗狀態和消息記錄
    const savedMessages = localStorage.getItem('chatMessages');
    const savedChatStatus = localStorage.getItem('isChatOpen');
    const authToken = localStorage.getItem('authToken');

    if (authToken) {
      // 自動使用 token 進行身份驗證
      axios.defaults.headers.common['Authorization'] = `Bearer ${authToken}`;
      // 你可以在這裡調用 API 來檢查用戶狀態，確保用戶仍然登入
    }
    ...
    if (savedChatStatus) {
      this.isChatOpen = JSON.parse(savedChatStatus);
    }
  },
  methods: {
    toggleChat() {
      this.isChatOpen = !this.isChatOpen;
      localStorage.setItem('isChatOpen', JSON.stringify(this.isChatOpen)); // 每次改變狀態時保存
    },
    async sendMessage() {
      // 添加用戶輸入到消息列表
      if (!this.userInput) return;
      this.messages.push({ role: 'user', content: this.userInput });
      localStorage.setItem('chatMessages', JSON.stringify(this.messages)); // 保存消息到 localStorage
