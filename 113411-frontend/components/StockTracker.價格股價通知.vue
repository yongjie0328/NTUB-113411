  ...
  <div class="stock-tracker">
    ...
    <form @submit.prevent="trackStock">
      ...
      <div class="form-group">
        ...
        <input type="email" id="userEmail" v-model="userEmail" placeholder="輸入您的電子郵件"><br>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      additionalStocksCount: { volume: 0, eps: 0, changePercentage: 0, institutionalVolume: 0 },
      url: 'https://notify-bot.line.me/zh_TW/',  // 這裡可以放你的連結
      linkText: 'LINE NOYIFY 申請'        // 每個排序類型的額外股票數量
    };
  },
  computed: {
    sortedStocks() {
  ...
  mounted() {
    fetch('http://127.0.0.1:8081/get_stocks')
      .then(response => {
        if (!response.ok) {
  ...
  methods: {
    trackStock() {
      ...
      fetch(`http://127.0.0.1:8081/track?ticker=${selectedTicker}&target_price=${this.targetPrice}&email=${this.userEmail}&line_notify_token=${this.lineNotifyToken}`)
        .then(response => {
          if (!response.ok) {
