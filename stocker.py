...
class Stocker():
    ...
    def __init__(self, price, training_years=3):  # 添加 training_years 參數，並設置默認值為 3
        self.symbol = 'the stock'
        self.training_years = training_years  # 初始化時設置 training_years
        s = price
    ...
    # Prophet model for specified number of days
    def create_prophet_model(self, days=0, resample=False):
        # 創建 Prophet 模型
        model = self.create_model()

        # 使用過去 self.training_years 年的數據進行預測
        stock_history = self.stock[self.stock['Date'] > (self.max_date - pd.DateOffset(years=3))].copy()

        if resample:
            stock_history = self.resample(stock_history)

        # 確保 stock_history 的 'ds' 欄位已經移除了時區
