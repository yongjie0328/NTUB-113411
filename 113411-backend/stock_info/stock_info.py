...
try:
    for market_type in market_types:
        ...
        if market_type == '上市':
            ...
            csv_file_path = "./stock_info/otc_stock_info.csv"
        ...
        if market_type == '上市':        
            ...
            # 新的 CSV 檔案路徑
            sii_stock_info_file_path = "./stock_info/sii_stock_info_select.csv"

            # 將選擇的資料存入新的 CSV 檔案
            selected_columns.to_csv(sii_stock_info_file_path, index=False, encoding='utf-8-sig')

            print(f'選擇的{market_type}資料已成功儲存到', sii_stock_info_file_path)
        else:
            df = pd.read_csv(csv_file_path)
            otc_column_names= ["股票代碼", "公司名稱", "公司簡稱", "產業類別", "住址", "董事長", "成立日期", "上櫃日期", "普通股每股面額", "實收資本額(元)", 
                            "已發行普通股數或TDR原發行股數", "編製財務報告類型", "普通股盈餘分派或虧損撥補頻率", 
                            "普通股年度(含第4季或後半年度)現金股息及紅利決議層級", "股票過戶機構", "簽證會計師事務所", "英文簡稱", "電子郵件信箱", "公司網址"]
            # 選擇指定的欄位
            selected_columns = df[otc_column_names]

            # 新的 CSV 檔案路徑
            otc_stock_info_file_path = "./stock_info/otc_stock_info_select.csv"

            # 將選擇的資料存入新的 CSV 檔案
            selected_columns.to_csv(otc_stock_info_file_path, index=False, encoding='utf-8-sig')

            print(f'選擇的{market_type}資料已成功儲存到', otc_stock_info_file_path)
