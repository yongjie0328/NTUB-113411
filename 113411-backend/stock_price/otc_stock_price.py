...
#櫃買中心個股日成交資訊
def get_tw_stock_data(start_year, start_month, current_year, current_month, month_list, stock_code):
    df = pd.DataFrame()
...
df = pd.read_csv(r"C:\Users\yingh\Desktop\python爬蟲\stock_info\otc_stock_info_select.csv", usecols=['公司代號'])
df.to_csv('otc_stock_id.csv', index=False, encoding='utf-8-sig')
...
#             print(f"找不到股票代碼 {stock_code} 的数据：{e}")
#             continue
...
df.to_csv('otc_stock_price.csv', index=False, encoding='utf-8-sig')
print("股票代碼已儲存到 otc_stock_price.csv 檔案中。")

# 讀取 otc_stock_price.csv 檔案
df = pd.read_csv('otc_stock_price.csv')
...
query_data.to_csv('otc_stock_price_selecct.csv', index=False, encoding='utf-8-sig')
print("所選日期範圍的資料已儲存到 otc_stock_price_selecct.csv 檔案中。")