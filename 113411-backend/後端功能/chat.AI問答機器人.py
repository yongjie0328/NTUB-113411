...
# 設定 OpenAI API 金鑰
openai.api_key = 'sk-proj-GBzrVZkGUdn7k34WLOkKT3BlbkFJ0YMhj9fqaRfBPsN31rM1'
...
# 使用 NewsAPI 查詢新聞並進行情緒分析
def get_stock_news_and_sentiment(query, api_key='7ec6753ed8a0451bbca566c6dd043b23'):
    url = f"https://newsapi.org/v2/everything?q={query}&apiKey={api_key}"
    response = requests.get(url).json()

    if response.get('status') != 'ok' or 'articles' not in response:
        return "無法獲取新聞資料，請稍後再試。"
    ...
    if sentiments:
        ...
        sentiment_description = "正面" if avg_sentiment > 0 else "負面" if avg_sentiment < 0 else "中立"
        return f"根據最近的新聞分析，該股票的市場情緒趨勢為 {sentiment_description}。"
    else:
        return "無法分析新聞情緒。"

# 定義 FastAPI 路由來處理前端的問答請求
@ask_router.post("/question", summary="回答問題")
async def ask_question(request: QuestionRequest):
    ...
    if knowledge_base_answer:
        return {"answer": knowledge_base_answer}
    ...
    if "股價" in user_question:
        ...
        if symbol:
            ...
            news_sentiment = get_stock_news_and_sentiment(symbol)
            return {"answer": f"{stock_price_answer}\n{news_sentiment}"}
